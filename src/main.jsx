  ...
  return <>
    <div className="login-shell">
      <section className="login-hero">
        <div className="eyebrow">DeutschOS Agent Demo · 德国硕士申请工作台</div>
        <h1>德国硕士申请 Agent 工作台</h1>
        <p>DeutschOS 覆盖申请者建档、项目匹配、顾问复核、风险监控与小浣熊 AI 辅助文书，帮助把德国硕士申请从经验驱动升级为可核验、可追踪、可复用的协同流程。</p>
        <div className="flow-strip"><span>申请者建档</span><b>→</b><span>AI 建议</span><b>→</b><span>顾问复核</span><b>→</b><span>管理者质控</span></div>
      </section>
...
function V04ResultPanel({ result }) {
  ...
  return <section className="panel v04-panel">
    ...
    <div className="grid-4 compact">
      ...
      <article className="metric"><span>AI 引擎</span><b>{aiOnline ? '小浣熊 AI' : '规则兜底'}</b><small>{aiOnline ? '服务端安全代理已接入' : '未接通时使用本地演示引擎'}</small></article>
    ...
    <div className={`ai-status-card ${aiOnline ? 'online' : 'fallback'}`}>
      <div><span>AI 状态</span><b>{aiOnline ? '小浣熊 AI 已接入' : '本地规则兜底'}</b><small>{aiAdvice.provider || 'local-expert-engine'} · {aiAdvice.model || 'demo-rules'}</small></div>
      <p>{aiOnline ? '生成方案已通过服务端安全代理调用 AI 建议引擎，API Key 不会暴露给浏览器。' : '当前仍可演示完整流程；配置服务端 AI Key 后会自动切换为在线 AI 建议。'}</p>
    </div>
...
function AdminConsole({ portalStatus, authStatus, refreshStatus, questions, setQuestions, runResult, portalData, portalMode }) {
  ...
  const openQuestionCount = (questions || []).filter(q => !['answered', 'closed', '已回复', '已关闭'].includes(q.status)).length;
